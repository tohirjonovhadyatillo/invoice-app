...
function DeleteModal({ invoiceId, onDeleteButtonClick, setIsDeleteModalOpen }) {
    const handleDelete = () => {
        ...
        toast.success(`Invoice ${invoiceId} successfully deleted`);
            ...
            <div className="scrollbar-hide overflow-y-scroll max-h-[95vh] my-auto bg-white dark:bg-[#2b2c37] text-black dark:text-white font-bold shadow-md shadow-[#364e7e1a] max-w-md mx-auto w-full px-8 py-8 rounded-xl">
                ...
                <div className="flex w-full mt-4 items-center justify-center space-x-4">
                    <button
                        ...
                        className="w-full text-white hover:opacity-75 bg-red-500 py-2 rounded-full"
                    >
                        Delete
                    </button>
                    <button
                        onClick={() => setIsDeleteModalOpen(false)}
                        className="w-full text-[#635fc7] dark:bg-white hover:opacity-75 bg-[#635fc71a] py-2 rounded-full"
                    >
                        Cancel
                    </button>
                </div>
